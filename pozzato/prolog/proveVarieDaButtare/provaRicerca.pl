...
ida_star(InitialS, MoveList, Visited):-
    ...
    NBound is Bound + 1,
    \+ida_star_loop(InitialS, MoveList, Visited, 0, Bound, [NBound]).   % se mettiamo una negazione per fallimento otteniamo true
...
ida_star_loop(InitialS, MoveList, Visited, G, Bound, CostList) :-
    ...
    NBound is NewBound + 1,
    ida_star_loop(InitialS,MoveList, Visited, 0, NewBound, [NBound]).
